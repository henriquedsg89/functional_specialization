...
object LineOfSight {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    def inner(idx: Int, max: Float): Unit = {
      if (idx >= input.length)
        return
      val newAngle: Float = input(idx) / idx
      val newMax = if (newAngle > max) newAngle else max
      output(idx) = newMax
      inner(idx + 1, newMax)
    }
    output(0) = input(0)
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    def innerUpsweep(idx: Int, max: Float): Float =
      if (idx == until) max
      else {
        val newMax = if (input(idx) > max) input(idx) else max
        innerUpsweep(idx + 1, newMax)
      }

    innerUpsweep(from, max = 0)
  }
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float], startingAngle: Float, from: Int, until: Int): Unit = {
    def innerTrav(idx: Int, maxAngle: Float): Unit = {
      if (idx == until)
        return
      val calcAngle = input(idx) / idx
      val newMax = if (calcAngle > maxAngle) calcAngle else maxAngle
      output(idx) = newMax
      innerTrav(idx + 1, newMax)
    }

    innerTrav(from, startingAngle)
  }
